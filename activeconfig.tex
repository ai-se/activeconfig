...
{\bf XXX vivek: remove WHERE from figures. add $S_i$}
...
XXX Vivek. what is the difference between baseline and exempler. both use all the data?
...
{\bf XXX VIVEK: (1) are the WHERE random results at X=0.4 the same values as those on \fig{my-label};
(2) pls change x axis to 20,40,60,80; (3) i still don't get the difference between where exemplar
and baseline given that baseline samples all}
...
\ei
{\bf XXX VIVEK: need a log scale on the y-axis of \fig{Evaluations}}.
