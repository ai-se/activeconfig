...
Researchers have developed number of approaches to tackle the problem of  large configuration spaces to meet the challenges of performance prediction and optimization.   
...
A more balanced approach by Siegmund et al.\ has an ability to learn predictors for  configurable systems~\cite{siegmund2012predicting} with low mean errors, but with large variances of prediction accuracy  (e.g.\ in half of the results, the performance predictions for the Apache Web server were up to 50\,\% wrong). 
...
We denote the number of features in the system (S) as n and all the features on S  can be represented by a boolean/binary space $\mathbb{Z}_{2}^{n}$, which is denoted by F. All the valid configuration of system S belong to a set V, which is represented by vectorrs $\vec{C_i}$ ($1\leq i\leq \left\vert{V}\right\vert$) belongs to Z. Each element of the configuration represent a feature which can either be \em{True} or \em{False} based on whether the feature was selected or not. 
...
Each valid instance of the vector (i.e., a configuration) has a corresponding performance score associated to it. 
...
Since each configuration ($\vec{C_i}$) is a vector of $n$ Booleans, it  is possible to use this information to isolate examples of how much each feature individually contributes to the total run time. The algorithm follows:
\begin{compactenum}
\item Find a pair of  configurations $\vec{C_i}$ and $\vec{C}_2$, where $\vec{C}_2$ uses exactly the same features as $\vec{C_i}$, plus just one  extra feature $f_i$.
...
Further to the first point, when many pairs such as ${\vec{C_1},\vec{C}_2}$ satisfying the criteria of point~1, Siegmund et al.\ used the 
...
\item[{\em HO (higher-order):}] Select extra configurations, in which three features, $f_1,f_2,f_3$, are selected if two of the following know pair-wise interactions exist: $(f_1,f_2)$ and $(f_2,f_3)$ and $(f_1,f_3)$.
...
only a few eigenvectors e, $e \ll d$  are required to characterize that data.
...
However, to the best of our knowledge, spectral methods have not been used before in software engineering (SE) as a basis of a minimal sampling method.
...
\item Find the point {\em East}~$\in N$
...
% \todo[inline]{What is non-numeric? Categorical? Nominal/ordinal or completely different types?}
...
\what& =  & WHERE $+$ Sampling-Policy $+$ CART
\end{tabular}
\end{center}
...
be very inaccurate and unstable, that is, it may exhibit very large mean prediction errors and standard deviations on the prediction error.
...
\ei
To answer this question, we will compare \what 
...
%
\multicolumn{4}{p{.95\linewidth}}{
\textbf{Berkeley DB Java Edition (BDBJ)} is a complete re-development in Java with full SQL support. Similarly, we used a benchmark provided by the vendor measuring response time.}\\
%
\multicolumn{4}{p{.95\linewidth}}{
...
{\bf RQ3}   used the regression-tree learned in this way as a {\em surrogate model} within an optimizer, as follows: 
\bi
...
\begin{figure}[!t]
\includegraphics[width=3.5in]{Figures/SamplingAccuracy.eps}
...
using these measurements. The horizontal axis of those plots shows what $X$\,\%
...
\item
The other lines show the results using the sampling methods defined in Section \ref{sample}.
...
\item
Mostly, the {\em exemplar} sampling method $S_3$ shows the highest overall error 
...
the $S_1$:Random policy ($S_2$ uses {\em two} samples per leaf cluster  while
...
using just a few dozens examples
of performance scores; 
and (b)~those predictions have error rates
within 5\,\% of the error rates seen if predictors are built from information about all performance scores.
\end{myshadowbox}

\begin{figure}[!t]
\includegraphics[width=3.5in]{Figures/evaluation_graph.eps}
\caption{ Comparing evaluations of different sampling policies. We see that the number of configurations evaluated for $S_2$:EastWest is twice as much as $S_1$:Random, since it selects 2 points from each cluster where as  $S_1$:Random selects only 1 point. }\label{fig:Evaluations}
\end{figure}
...
\begin{figure}[!t]
\includegraphics[width=3.5in]{Figures/Variance.eps}
...
configurations, 20 times, then relate that baseline to three optimizers (GALE~\cite{krall2014gale}, DE~\cite{storn1997differential} and  NSGA-II~\cite{deb00afast}) using their
default parameters.
...
used only a small part of that ground truth data in their search for the fastest configurations (see the \what + $S_1$:Random
results of \fig{Evaluations}).


\begin{figure}[!t]
\includegraphics[width=3.5in]{Figures/optimizer_result.eps}
...
configuration in the ground truth (see all the left-hand-side dots on each plot). Table~\ref{fig:external_validity} compares the performance of the optimizers
...
\hline
\multirow{3}{*}{\textbf{Dataset}} & \multicolumn{6}{c|}{\textbf{Searcher}}                                                                       \\ \cline{2-7} 
                                  & \multicolumn{2}{c|}{\textbf{GALE}} & \multicolumn{2}{c|}{\textbf{DE}} & \multicolumn{2}{c|}{\textbf{NSGAII}} \\ \cline{2-7} 
                                  & \textbf{Mean}    & \textbf{IQR}    & \textbf{Mean}   & \textbf{IQR}   & \textbf{Mean}     & \textbf{IQR}     \\ \hline
\textbf{Apache}                   & 870              & 0               & 840             & 0              & 840               & 0                \\ \hline
\textbf{BDBC}                     & 0.363            & 0.004           & 0.359           & 0.002          & 0.354             & 0.005            \\ \hline
\textbf{BDBJ}                     & 3139             & 70              & 3139            & 70             & 3139              & 70               \\ \hline
\textbf{LLVM}                     & 202              & 3.98            & 200             & 0              & 200               & 0                \\ \hline
\textbf{SQLite}                   & 13.1             & 0.241           & 13.1            & 0              & 13.1              & 0.406            \\ \hline
\textbf{X264}                     & 248              & 3.3             & 244             & 0.003          & 244               & 0.05             \\ \hline
\end{tabular}
}
...
runs. Mean values are denoted $\mu$ and IQR denotes the 25th--75th percentile. Low IQR suggests that the surrogate model build by \what is stable and can be utilized by off the shelf optimizers to find performance-optimal configurations.
...
we cannot recommend this method or  if one wishes to use this method we recommend using the other sampling heuristics (e.g., HO, HS) to make more accurate predictions, but at the cost of more measurements. Moreover, the size of the standard deviation of this method causes further difficulties in estimating which configurations are those exhibiting a large prediction error. 
