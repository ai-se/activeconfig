...
predictions about, say, the expected software performance scores of planned products. 
...
Siegmund et al. in 2012 built predictors for a product line description of the Apache web server~\cite{siegmund2012predicting}. Their resulting predictor had moderately low mean errors   but the predictions
 were  so  unstable\footnote{The predictions of   \cite{siegmund2012predicting} had $\mu=19\%$ errors with
 $\sigma = 19.5\%$.}
...
For the \cite{siegmund2012predicting} errors, that range is  0 to 49\%.}. Guo et al.~\cite{guo2013variability} also proposed an incremental method to build a predictor model, which uses incremental random samples with steps equal to number of features of the software system. It suffered from the same problem, as the previous work, of unstable predictions\footnote{The predictions of \cite{guo2013variability} had  $\my=22\%$ errors with $\sigma=46\%$}.
...
the  predictions are far more accurate with much lower variance than those of Siegmund et al. and Guo et. al \footnote{The prediction of in this paper have $\mu=6\%$ error with $\sigma=2\%$.}.
...
This is very significant and most surprising,  since some of the product lines explored here have up
to millions of possible variations. This result is very useful to the community, since data collection in case 
...
consequences of all the configuration options (for ex. such as \what).


The contributions of this paper are:
\bi
\item A new effective sampling strategy for predicting performance metrics of software systems from feature/variability models. The sampling strategy uses an approximate to the first principal component of feature space to recursively cluster and only use only a few points as a representative of each cluster;
\item We implement the approach and demonstrate its practicality and generality by conducting experiments on six real world configurable software systems (see ~\ref{fig:systems}). The results show that our approach is far more accurate (lower mean error) and much more stable(lower standard deviation);
...
In this section, we formalize the problem of predicting performance scores of software systems. All the software systems (configurable) has a set of $X$ boolean configurations or features also referred to as independent  variables. If a feature is selected in a configuration corresponds to $True$,  $False$ otherwise. All the features of a system is represented as a vector F= <$f_1$,$f_2$, ..., $f_N$>, where $N$ is the number of features of the software systems. Each of the vector has a corresponding performance score associated to it. The software system used to validate our method are described in Figure ~\ref{fig:systems}.
Note, ``predicting performance scores'', we 
...
As shown in the Figure~\ref{fig:bdbc} a feature model can be presented as a tree like structure that defines relationships among features.   Features are represented as a set of boolean decision variables. 

A feature, when selected, corresponds to True, and False otherwise. All the features of a system is represented a {\em configuration} vector  $C_i= <f_1, f_2, ...,f_N>$, 
...
that  explore the spectrum (eigenvalues) of the distance matrix between  configurations.
...
our key contribution presented in this paper  (defining
 a minimal sampling policy to predict performance scores).
Also,
...
After collecting the data using one of the sampling policies, we then use the CART regression tree learner \cite{breiman1984} to build a predictor for performance scoress. Regression tree learners seek the attribute range split that most increases
...
configuration that has (say) faster performance scores. Such optimizers might
...
\bi
\item Performance scores (performance scores) generated from our minimal sampling policy;
...
learning performance score predictors from product lines, how good is \what with $S_1$:Random?}
\ei
...
the answer to {\bf RQ1} is ``yes'' since using \mbox{\what + $S_1$:Random} we can (a)~generate runtime predictors
using just a few dozens examples of performance scores; and (b)~those predictions have errors
within 5\% of the error seen if predictors built from information about all performance scores.
\end{myshadowbox}
...
runtime predictions.
The   less we sampl} the configuration space,
...
\caption{Solutions found by GALE (shown as points) laid against the ground truth (all known configuration performance scores) with the exception of SQLite, as this dataset does not contain the exhaustive set of configurations and the corresponding performance scores.}\label{fig:performance_graph}
\end{figure}
