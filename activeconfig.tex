...
\end{tabular}
\caption{Spectral Learning using \what{} using the  example of a system with two configuration options.}
\label{fig:spectral_desc}
\end{figure}



Figure~\ref{fig:spectral_desc} describes the procedure used to calculate the projection of a configurations. \what receives $N$ (with $1\leq \left\vert{N}\right\vert\leq \left\vert{V}\right\vert$)
...
\what{} requires that the practitioner to define a stopping criterion, (size of the sub-region) before the
process commences.
...
\subsection{\respto{2a}~What is the relationship between intrinsic dimensionality and difficulty of a problem (or dataset)?}

\textcolor{blue}{
Houle et al.~\cite{houle2012generalized} observe a clear correlation between dimensionality of a problem space and loss of performance, i.e. problem represented in lower dimensions is easier to model than the same problem represented in higher dimensions. On a similar vein, Domingos~\cite{domingos2012few} explains how our intuitions fail in higher dimensions or algorithms which work on lower dimensions does not work on higher dimensions. This is because the size of the training data required to create a generalized 'model' for such a high dimensional space is exponentially large~\footnote{Another challenge of having high dimensional search space is the amount of noise induced by irrelevant dimensions.}. This is generally referred to as the curse of dimensionality, but what counteracts this is the 'blessing of non-uniformity'. 'Blessing of non-uniformity' refers to the fact that the possible valid solutions in the space is not spread uniformly across the problem space but concentrated on or near a lower dimensional manifold.  Hence, it is a rule of thumb of machine learning practitioners to reduce the dimension of data by projecting the data onto a lower dimensional orthogonal subspace that captures the variation in the data. Bruges~\cite{burges2010dimension} mentions that if data lies in a lower dimensional space (with lower intrinsic dimensions) then modeling the data directly in lower dimensional manifold make it much easier to model. Our results attest to this observation made by Bruges, as we show that few samples are enough to model a  large (sometimes millions) space, which can be attributed to the low intrinsic dimensionality of the space. There are several other techniques, which have been used to exploit the non-uniformity of the solutions such as Random Projections~\cite{dasgupta2000experiments}, Auto-encoders~\cite{hinton2006reducing}. Thus, problems with lower intrinsic dimensionality is easier/cheaper to model than problems with higher intrinsic dimensionality. To the best of our knowledge, we are the first to notice the lower intrinsic dimensionality of the configuration space and exploit it for sampling.
}
...
\review{A few minor things to consider for your final version below.   Please make \& summarise any suggested changes/rebut and I will check the final version myself before making a final recommendation to the Editor in Chief.}
...
\subsection*{\textbf{Response to Reviewer \#2}}
\noindent
\review{In my opinion the authors should cite more general papers discussing the effect of low intrinsic dimensionality as a measure of learning easiness, and therefore, suggest that their approach seems to be the first to take advantage of this fact in the data sets studied. For example, the work of Mike Houle, whose first paper on intrinsic dimensional is
Michael E. Houle, Hisashi Kashima, Michael Nett:
Generalized Expansion Dimension. 
}\\
\noindent
We think is an important addition to the paper and can provide an intuition to this methods used in \what. To address this we have added a new discussion subsection~\citeresp{2a}.\\

% \noindent
% We agree with you and hence we add a new section~\citeresp{2e}.\\

\noindent
\review{There are a few writing issues like a missing closing parenthesis ")" in caption of Figure 1. Also, Line 33-34 in Page 27, odd break of line use tilde in LaTeX so the word "Figure" does
not appear in a different line than the numeral "8"
}\\
\noindent
Thank you for pointing it out. The closing parenthesis is added to Figure 1 and odd line break in Page-27 is fixed.\\

% \noindent
% \review{ I would also like a discussion of what exactly "performance" means - this could be CPU utilization, throughput, \# users}\\

% \noindent
% This is definitely a good point. Our previous version of the paper did not mention these details in an accessible format. In the revised version we have added a detailed table mentioning the details about how the data is collected and clearly define how performance is measured~\citeresp{2b}.\\

% \noindent
% \review{why each chosen?}\\
% \noindent
% We choose our subject systems from different domains with different configuration mechanism to cover different systems which are used in real-world. We have explicitly mentioned it in~\citeresp{2c} as well as in the threats to validity section (external validity).


% \review{
% \begin{itemize}
% \item What the benchmark/performance assessment used actually is - this is critical in determining the parameter settings and at the moment I have no idea what each measures/how it works for each application. I suspect they may be very different?
% \item some sense of the configuration parameters - a few examples would actually be nice so the reader can get a feel of the sort of configuration parameters being set
% \end{itemize}
% }\\
% \noindent
% To address this we have added a new section~\citeresp{2a}. \\

% \noindent
% \review{whether all these parameters actually impact performance - I suspect not - but can't tell what they are so can't make a judgement. I do wonder if like is being compared with like though?}\\
% \noindent
% Regarding the importance of different configurations, we did not make any assumptions on whether some or all configurations are important. We assumed that all configurations are of equal importance and perform no pre-processing on the data.\\

% \noindent
% \review{why the huge \# configurations for SQLite vs others - was not clear from the discussion.}\\
% In this new draft, we add the reason why SQLite has huge number of configurations~\citeresp{2d}.\\

% \noindent
% \review{It would be good, as above, to somewhere discuss the limitations of the technique. There must be some - would be good to more clearly lay these out for the reader.}\\

% \noindent
% This is an important point and we did not answer it explicitly in the text. To address this we have added a new section~\citeresp{2a}.\\

% \noindent
% \review{what is the key future work that needs doing? Why?}

% \noindent
% As per your suggestion, we have mentioned the key future work~\citeresp{2f}.\\

% \noindent
% \review{I would have liked some details about how the technique is actually implemented in section 3 - what tools used to build, how they fit together and so on. I got no sense at all from the paper what implementation challenges result from building the tool and using it.}\\

% \noindent
% We mention this in section 5.3.
