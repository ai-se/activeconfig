...
The minimal sampling method explored in this paper is based on spectral clusterers
...
When recursive spectral clustering terminates, our minimal sampling policy (which we will call $S_1$) is then applied:
\begin{quote}
...
and found that $S_1$ performs better other sampling methods:
\bi
\item $S_2$: East-West sampling: compile and execute the {\em East,West} poles of the leaf clusters;
\item $S_3$: Exemplar sampling: compile and execute the centroid of each leaf cluster;
\ei
Using the data collected this way, we then use the CART decision tree learner \cite{breiman1984} to build a predictor for runtimes. 
This predictor is then tested on data {\em not} used in the above spectral clustering processing (see below for details). For each each test item, we find out how long it {\em actually} takes to run and compare that
...
into a runtime prediction model~\cite{breiman84}.
That is,
\begin{center}
