...
not just the smallest $\vec{C}_1,\vec{C}_2$  pairs, but also any configurations with {\em interactions} between features. 
...
\todo[inline]{In the remaining text we dont need to mention S1:Random, as it is part of \what, be the above definition. Since we compare different sampling techniques in first research question, I think it is necessary to mention the sampling policy along with \what}
...
runs. Mean values are denoted $\mu$ and IQR denotes the 75th--25th percentile. Low IQR suggests that the surrogate model build by \what is stable and can be utilized by off the shelf optimizers to find performance-optimal configurations.
